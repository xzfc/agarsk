#include <fstream>
struct Svg {
	Svg(const char *filename, Aabb cords, unsigned w, unsigned h)
	: f(filename) {
		f << "<svg xmlns='http://www.w3.org/2000/svg' viewBox='"
		  << cords.x0 << " " << cords.y0 << " "
		  << cords.x1 - cords.x0 << " " << cords.y1 - cords.y0
	...
	void circle(double x, double y, double r, const char *stroke, const char *fill) {
		f <<
		"  <circle "
		"r='" << r << "' "
		"cx='" << x << "' "
